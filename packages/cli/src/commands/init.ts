...
export async function initCommand(options: InitOptions): Promise<void> {
  ...
  p.intro(chalk.bgCyan.black(' AI Agentic Architecture '));
  ...
  console.log('');
  console.log(`To update later: ${chalk.cyan('npx @tutellus/agentic-architecture update')}`);
  } catch (err) {
...
async function showPreview(
  ...
  if (selectedIDEs.includes('claude-code')) {
    ...
    console.log(chalk.dim('     └── Skills: /tut-ai:brief, /tut-ai:spec, /tut-ai:research, ...'));
