 ...
 * ordering (FR-1, US-1). Readiness is a PURE function over an injected fs-probe.
...
describe('backlog readiness (FR-1, FR-3)', () => {
  describe('US-1/AC: eligibility = spec + DoD both present', () => {
    it('marks an item ready when both its spec and DoD tests exist', () => {
      // Spec: FR-1, FR-3
      expect(checkReadiness(ITEM, readyProbe)).toMatchObject({ ready: true });
    });

    it('marks an item NOT ready with a reason when the spec is missing', () => {
      // Spec: FR-3
    ...
    it('marks an item NOT ready with a reason when the DoD tests are missing', () => {
      // Spec: FR-3
  ...
  describe('FR-3: skipped items are reported, never silently dropped', () => {
    it('eligibleQueue returns both the ready list and the skipped list with reasons', () => {
      // Spec: FR-3
...
describe('priority ordering (FR-1, US-1)', () => {
  it('orders eligible items by descending priority (higher first)', () => {
    // Spec: FR-1
    const items = [
  ...
  it('is stable within a priority band (FIFO for ties)', () => {
    // Spec: FR-1 (tie-break)
    const items = [
