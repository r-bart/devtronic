 ...
 * is a predicate over counters. Greenfield — `../backlogPlan.js` does not exist yet.
 */
...
describe('buildBacklogPlan — dry-run (FR-2, US-4)', () => {
  it.todo('previews eligible items in priority order, annotated, executing nothing');
  // Spec: US-4
  // const lines = buildBacklogPlan(QUEUE, { budget: 500_000, width: 3 });
  // expect(lines.map(l => l.itemId)).toEqual(['b', 'a']);

  it.todo('states the budget and width caps in the plan');
  // Spec: FR-7, US-4
  // const lines = buildBacklogPlan(QUEUE, { budget: 500_000, width: 3 });
  // expect(lines.some(l => /width|budget/i.test(l.explanation))).toBe(true);

  it.todo('is pure — reads/writes no filesystem and does not exec');
  // Spec: US-4 (dry-run purity)
  // spy on fs / child_process -> not called while building the plan.
});

describe('canLaunchNext — budget + width gate (FR-7)', () => {
  it.todo('allows launch when under both the budget and the width cap');
  // Spec: FR-7
  // expect(canLaunchNext({ budgetSpent: 100_000, budgetTotal: 500_000, inFlight: 1, widthCap: 3 })).toBe(true);

  it.todo('blocks launch when the width cap is reached');
  ...
  it.todo('blocks launch when the outer budget is exhausted');
