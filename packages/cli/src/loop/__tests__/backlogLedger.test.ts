 ...
 * round-trip so `--resume` can reconstruct state faithfully. Greenfield —
 * `../backlogLedger.js` does not exist yet.
 */
...
describe('run ledger (FR-10)', () => {
  it.todo('serialize then parse round-trips the ledger faithfully (resume relies on it)');
  // Spec: FR-10, FR-11
  // expect(parseLedger(serializeLedger(LEDGER))).toEqual(LEDGER);

  it.todo('records each item state: done, parked, quarantined');
  // Spec: FR-10
  // const parsed = parseLedger(serializeLedger(LEDGER));
  // expect(parsed.items.map(i => i.state).sort()).toEqual(['done', 'parked', 'quarantined']);

  it.todo('carries a per-item pointer to the inner trace and a budget-spent total');
  // Spec: FR-10
  // const parsed = parseLedger(serializeLedger(LEDGER));
