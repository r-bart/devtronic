...
// ─── Addon source path (will be set during implementation) ──────────────────
// The addon content is bundled with the CLI package.
// This path will need updating once the addon directory structure is created.

// For now, define the expected path relative to packages/cli/src
// This will be: packages/cli/src/addons/design-best-practices/
...
describe('design-best-practices addon structure', () => {
  it.todo('FR-1: addon source directory exists');
  // Spec: FR-1
  // expect(existsSync(ADDON_ROOT)).toBe(true)

  it.todo('FR-2: manifest.json exists and is valid JSON');
  // Spec: FR-2
  ...
  it.todo('FR-2: manifest declares all 5 skills');
  // Spec: FR-2, FR-6
  ...
  it.todo('FR-2: manifest declares all 7 reference docs');
  // Spec: FR-2, FR-7
  // const manifest = JSON.parse(readFileSync(join(ADDON_ROOT, 'manifest.json'), 'utf-8'))
  ...
  it.todo('FR-2: manifest declares design-quality rule');
  // Spec: FR-2, FR-8
  // const manifest = JSON.parse(readFileSync(join(ADDON_ROOT, 'manifest.json'), 'utf-8'))
  // expect(manifest.files.rules).toEqual(['design-quality.md'])

  it.todo('FR-2: manifest includes attribution for Apache 2.0 content');
...
describe('design addon skill files', () => {
  ...
  it.todo('FR-6: design-system skill supports extract and normalize modes');
  // Spec: US-9/AC-4
...
describe('design addon reference docs', () => {
  ...
  for (const ref of expectedRefs) {
    it.todo(`FR-7: reference/${ref} exists`);
    // Spec: FR-7
    // const refPath = join(ADDON_ROOT, 'reference', ref)
    // expect(existsSync(refPath)).toBe(true)
  }

  it.todo('FR-7: typography.md covers type scales and fluid sizing');
  // Spec: FR-7
  ...
  it.todo('FR-7: color-and-contrast.md covers OKLCH and WCAG');
  // Spec: FR-7
  ...
  it.todo('FR-7: motion-design.md covers reduced motion');
  // Spec: FR-7
